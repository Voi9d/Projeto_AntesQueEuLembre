init python:
    # (No-op) kept to avoid load-order issues. Revert logic is defined
    # in `characters.rpy`. Do not modify here unless necessary.
    pass
...
screen say(who, what):
    ...
    # Prevent mouse wheel from advancing or rolling back dialogue.
    # Mouse wheel events are reported as mouse buttons 4 and 5 on many
    # systems. Use those events instead of the invalid 'wheelup'/'wheeldown'.
...
style choice_button is default:
    ...
    hover_background Solid("#0ea5aacc")
    padding (24, 18)
    xminimum 800
...
screen quick_menu():
    ...
    if quick_menu:
        # Pressionar ESC (game_menu) abre diretamente Preferências durante o jogo
        ...
        hbox:
            ...
            # textbutton "Salvar" action ShowMenu("save") sensitive can_save
...
screen navigation():
    vbox:
        ...
        if main_menu:
            textbutton _("Novo Jogo") action ShowMenu("confirm", message=_("Você tem certeza de iniciar um novo jogo? Todos os seus dados salvos serão perdidos."), yes_action=[Function(reset_game_progress), Hide("confirm"), Start()], no_action=Hide("confirm"))
...
screen main_menu():
    ...
    # If there are unsaved codex changes from the previous session, discard
    # them when the main menu is shown (i.e. player confirmed returning
    # to the main menu). This ensures friendship changes made in-session
    # without saving are not persisted.
    python:
        try:
            if codex_unsaved_changes:
                try:
                    revert_unsaved_codex_changes()
                except Exception:
                    pass
        except Exception:
            pass
...
screen load():
    ...
    use file_slots(_("Carregar Save"))
        ...
        fixed:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    $ is_save_screen = (CurrentScreenName() == "save")

                    button:
                        action ([ FileAction(slot), Return() ] if is_save_screen else FileAction(slot))
